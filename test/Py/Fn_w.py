...
ax1.semilogy(jmask, df['jfrn']/df['jfrn'][0], marker = 's', color = 'green', label = 'Area', markersize = 2)
ax1.semilogy(jmask, np.abs(df['JEP'])/df['JEP'][0], marker = '^', color = 'blue', label = r'$j \cdot E^{(+)}$', markersize = 2)
ax1.semilogy(jmask, df['JEM']/df['JEM'][0], marker = '^', color = 'deepskyblue', label = r'$j \cdot E^{(-)}$', markersize = 2)
ax1.semilogy(jmask, df['pideP']/df['pideP'][0], marker = 'o', color = 'red', label = r'$PiD_e^{(+)}$', markersize = 2)
ax1.semilogy(jmask, df['pideM']/df['pideM'][0], marker = 'o', color = 'maroon', label = r'$PiD_e^{(-)}$', markersize = 2)

x1 = np.linspace(0.1, 3, 200)
y1 = np.exp(- x1**2)
ax1.semilogy(x1, y1, c = 'black', linestyle = 'dashed', linewidth = 3, label = r'$e^{-\omega^e}$')
...
ax.set_title(r'$t \omega_{ci} =$'f'{np.round(twci[t],2)}', fontsize = 20)
...
ax3.semilogy(jmask, df['jfrn']/df['jfrn'][0], marker = 's', color = 'green', label = 'Area', markersize = 2)
ax3.semilogy(jmask, np.abs(df['JEP'])/df['JEP'][0], marker = '^', color = 'blue', label = r'$j \cdot E^{(+)}/\langle j \cdot E^{(+)} \rangle$', markersize = 2)
ax3.semilogy(jmask, df['JEM']/df['JEM'][0], marker = '^', color = 'deepskyblue', label = r'$j \cdot E^{(-)}/\langle j \cdot E^{(-)}\rangle$', markersize = 2)
ax3.semilogy(jmask, df['pideP']/df['pideP'][0], marker = 'o', color = 'red', label = r'$PiD_e^{(+)}/\langle PiD_e^{(+)}\rangle$', markersize = 2)
ax3.semilogy(jmask, df['pideM']/df['pideM'][0], marker = 'o', color = 'maroon', label = r'$PiD_e^{(-)}/\langle PiD_e^{(-)}\rangle$', markersize = 2)
...
x1 = np.linspace(0.1, 3, 200)
y1 = np.exp(- x1**2)
ax3.semilogy(x1, y1, c = 'black', linestyle = 'dashed', linewidth = 3)
...
ax3.tick_params(labelleft = False, labelsize = 18)
ax1.set_xlim(0,3)
ax3.set_xlim(0,3)
ax.yaxis.offsetText.set_fontsize(12)

fig5.supxlabel(r'$|\omega^e/\omega^e_{rms}|$', size = 20)
fig5.savefig(figs + 'Fn_w.png', dpi = 200)
# fig.clf()