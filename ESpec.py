...
def EbSpec2D(dbx, dby):
    ...
    kfreqx = np.fft.fftshift(np.fft.fftfreq(nx) * nx * 2 * np.pi)
    kfreqy = np.fft.fftshift(np.fft.fftfreq(ny) * ny * 2 * np.pi)
    kfreq2D = np.meshgrid(kfreqx, kfreqy)
    knrm = np.sqrt(kfreq2D[0]**2 + kfreq2D[1]**2)
    knrm = knrm.flatten()
    
    kbins = np.linspace(0, 5 * int(np.max(knrm)), int(np.max(knrm)))
